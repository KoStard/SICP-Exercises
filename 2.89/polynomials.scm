...
(define (install-polynomial-package)
    ...
    (define (negate_term term)
        (make-term (order term) (- (coeff term)))
    )
    (define (negate_terms terms)
        (if (empty-termlist? terms)
            (the-empty-termlist)
            (adjoin-term (negate_term (first-term terms)) (negate_terms (rest-terms terms)))
        )
    )
    (define (negate p)
        (make-poly (variable p) (negate_terms (term-list p)))
    )
